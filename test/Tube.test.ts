...
describe("tube test", function () {
  ...
  beforeEach(async function () {
    ...
    ret = await factory.createForeignToken(CHAIN_ID, srcToken.address, "name", "symbol", 6)
  ...
  describe("deposit", function () {
    it("invalid amount", async function () {
      await expect(tube.deposit(CHAIN_ID, srcToken.address, 0)).to.be.revertedWith("invalid amount")
    })

    it("without fee", async function () {
      await tube.setFee(CHAIN_ID, 1000000)

      await expect(tube.deposit(CHAIN_ID, srcToken.address, 1000)).to.be.revertedWith("transfer amount exceeds balance")
    })
    ...
    it("success without fee", async function () {
      ...
      await expect(tube.deposit(CHAIN_ID, srcToken.address, 300000))
        ...
        .withArgs(CHAIN_ID, srcToken.address, 0, owner.address, owner.address, 300000, 0)
    ...
    it("success with fee", async function () {
      ...
      await expect(tube.deposit(CHAIN_ID, srcToken.address, 300000))
        .to.emit(tube, "Receipt")
        .withArgs(CHAIN_ID, srcToken.address, 0, owner.address, owner.address, 300000, 1000000)
  ...
  describe("withdraw", function () {
    ...
    it("amount is 0", async function () {
      await expect(
        tube.withdraw(CHAIN_ID, srcToken.address, 0, holder1.address, 0, ZERO_THREE_SIGNATURES),
      ).to.be.revertedWith("amount is 0")
    })

    it("invalid recipient", async function () {
      await expect(
        tube.withdraw(CHAIN_ID, srcToken.address, 0, ZERO_ADDRESS, 1000, ZERO_THREE_SIGNATURES),
      ).to.be.revertedWith("invalid recipient")
    })

    it("invalid signature length", async function () {
      await expect(tube.withdraw(CHAIN_ID, srcToken.address, 0, holder1.address, 1000, 0x00)).to.be.revertedWith(
    ...
    it("invalid validator", async function () {
      await expect(
        tube.withdraw(CHAIN_ID, srcToken.address, 0, holder1.address, 1000, ZERO_THREE_SIGNATURES),
      ).to.be.revertedWith("invalid validator")
    })

    it("duplicate validators", async function () {
      const key = await tube.genKey(CHAIN_ID, srcToken.address, 0, holder1.address, 1000)

      const s1 = sign(key.slice(2), VALIDATOR_PRIVATE_KEYS[0])
      const signature = "0x" + s1 + s1

      await expect(tube.withdraw(CHAIN_ID, srcToken.address, 0, holder1.address, 1000, signature)).to.be.revertedWith(
        "duplicate validator",
      )
    })
  ...
  describe("withdrawInBatch", function () {
    ...
    it("invalid signature length", async function () {
      await expect(
        tube.withdrawInBatch([CHAIN_ID], [srcToken.address], [0], [holder1.address], [100], "0x00"),
      ).to.be.revertedWith("invalid signature length")
    })

    it("invalid parameters", async function () {
      await expect(
        tube.withdrawInBatch([CHAIN_ID], [srcToken.address], [], [holder1.address], [100], ZERO_THREE_SIGNATURES),
    ...
    it("amount is 0", async function () {
      await expect(
        tube.withdrawInBatch([CHAIN_ID], [srcToken.address], [0], [holder1.address], [0], ZERO_THREE_SIGNATURES),
      ).to.be.revertedWith("amount is 0")
    })

    it("invalid recipient", async function () {
      await expect(
        tube.withdrawInBatch([CHAIN_ID], [srcToken.address], [0], [ZERO_ADDRESS], [100], ZERO_THREE_SIGNATURES),
      ).to.be.revertedWith("invalid recipient")
    })

    it("invalid validator", async function () {
      await expect(
        tube.withdrawInBatch([CHAIN_ID], [srcToken.address], [0], [holder1.address], [100], ZERO_THREE_SIGNATURES),
      ).to.be.revertedWith("invalid validator")
    })

    it("insufficient validators", async function () {
      const key1 = await tube.genKey(CHAIN_ID, srcToken.address, 0, holder1.address, 1000)

      const key2 = await tube.genKey(CHAIN_ID, srcToken.address, 0, holder2.address, 200)
